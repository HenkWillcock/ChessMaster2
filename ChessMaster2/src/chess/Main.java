...
public class Main 
{
	...
	public Main()
	{
		...
		controller = new Controller(board,graphicsHandler, 50, 50, 75, 75);
	...
	private void setHumanPlayerColour(String colour)
	{
		...
		{
			movement = board.getMovement();
			controller.setAI(new AI(board, colour2, movement));
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	public void setWindowList()
	{
		try 
		{
			this.controller.setGameMode(gameMode);
		}
		catch(Exception e)
		{
			e.printStackTrace();
	...
	private class GameLoaderWindow extends JFrame
	{
		...
		public void CloseFrame()
		{
			...
			try 
			{
				Main.this.controller.setGameMode(game.mode);
				if(game.mode == 1)
				{
