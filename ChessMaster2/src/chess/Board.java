...
public class Board {
	...
	private CopyOnWriteArrayList<Piece> whitePieces, blackPieces, killedPieces;
	...
	public Board(Boolean x) {
		...
		try { // giving 2 bishops to both players on cells just beside the knights.
	...
	/**
	 * Returns the reference to the cell at (row, col) in the board. Returns null if
	 * the row or column are not valid (out of bounds).
	 */
	...
	/**
	 * This method just prints current board situation to terminal. Used for
	 * debugging.
	 **/
	public void print() {
		for (int i = 0; i < 8; i++) {
	...
	/**
	 * This function is called whenever the user clicks on the cell having this
	 * (row, col).
	 * 
	 * It highlights the clicked cell if none is highlighted currently.
	 * 
	 * If some cell is highlighted, it moves the piece on that cell to this cell.
	 * 
	 * @return true if a move is made, Returns false if either the cell is empty, or
	 *         contains an opponent piece or is not a valid destination.
	 */
	...
	/*
	 * public ArrayList<Cell> getAllMoves(Piece piece) { return
	 * movement.getAllMoves(piece); }
	 */
